/**
 * Importa estudiantes desde nómina Excel a Supabase.
 *
 * Uso:
 *   node scripts/import-nomina.mjs "archivo.xlsx" --nivel Primaria
 *   node scripts/import-nomina.mjs "archivo.xlsx" --nivel Secundaria --dry-run
 */
...
function parseArgs(argv) {
  ...
  const nivelIdx = args.findIndex((a) => a === '--nivel');
  const nivel =
    nivelIdx >= 0 ? args[nivelIdx + 1] : args.find((a) => VALID_LEVELS.has(a));
  const file = args.find((a) => !a.startsWith('--') && !VALID_LEVELS.has(a) && a !== nivel);

  if (!file || !nivel || !VALID_LEVELS.has(nivel)) {
    console.error(
      'Uso: node scripts/import-nomina.mjs <archivo.xlsx> --nivel Primaria|Secundaria [--dry-run]'
    );
    process.exit(1);
  }
  return { file: resolve(file), dryRun, nivel };
}
...
function toStudentRow(row, nivel) {
...
async function main() {
  loadEnvLocal();
  const { file, dryRun, nivel } = parseArgs(process.argv);
  ...
  const mapped = rawRows.map((r) => toStudentRow(r, nivel)).filter(Boolean);
