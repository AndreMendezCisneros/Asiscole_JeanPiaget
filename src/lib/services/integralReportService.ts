import { incidentsService, notasService, pensionesService, tallerAttendanceService } from '@/lib/services';
...
import type { NotasEstudianteHistorial } from '@/types/notas';
...
export type IntegralStudentBlock = {
  ...
  incidents: Incident[];
  notas: NotasEstudianteHistorial[];
...
function notesOverlapPeriod(nota: NotasEstudianteHistorial, start: string, end: string): boolean {
  return nota.fechaFin >= start && nota.fechaInicio <= end;
}
...
async function fetchNotasForStudents(
  students: Student[],
  start: string,
  end: string,
): Promise<Map<number, NotasEstudianteHistorial[]>> {
  const byStudent = new Map<number, NotasEstudianteHistorial[]>();
  if (!isNotasEnabled() || students.length === 0) return byStudent;
  ...
  for (let i = 0; i < students.length; i += concurrency) {
    ...
    const results = await Promise.all(
      batch.map(async (student) => {
        const { notas } = await notasService.listNotasEstudiante(student.id, 80);
        return {
          id: student.id,
          notas: notas.filter((nota) => notesOverlapPeriod(nota, start, end)),
        };
      }),
    );
    for (const result of results) {
      byStudent.set(result.id, result.notas);
    }
  }
  return byStudent;
}
...
export async function assembleIntegralBlocks(input: {
  ...
  const [notasByStudent, pensionesByStudent] = await Promise.all([
    fetchNotasForStudents(students, start, end),
    fetchPensionesForStudents(students, start, end),
  ]);
  ...
  const blocks = students.map((student) => {
    ...
    const studentNotas = notasByStudent.get(student.id) ?? [];
