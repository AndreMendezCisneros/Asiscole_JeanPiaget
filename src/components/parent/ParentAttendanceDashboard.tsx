...
import { arrivalService, fetchMonthIncidentsForStudent, incidentsService, tallerAttendanceService } from '@/lib/services';
...
export function ParentAttendanceDashboard({
  ...
  const loadMonth = useCallback(
    async (year: number, month: number) => {
      ...
      try {
        ...
        const [arrivals, tallerResponse, publicIncidents, staffIncidents] = await Promise.all([
          arrivalService.fetchMonthArrivalsForStudent(student.id, year, month, student.level),
          talleresEnabled
          ...
          fetchMonthIncidentsForStudent(student.id, year, month),
        ...
        const incidents =
          !staffIncidents.error && staffIncidents.incidents.length > 0
  ...
  useEffect(() => {
    ...
    if (isInitial) {
      ...
      void Promise.all([
        ...
        fetchMonthIncidentsForStudent(student.id, viewYear, viewMonth),
        incidentsService.getAll({
          estudianteId: student.id,
          fechaDesde: `${start}T00:00:00.000-05:00`,
          fechaHasta: `${end}T23:59:59.999-05:00`,
        ...
        .then(([attendanceResponse, publicIncidents, staffIncidents]) => {
  ...
  const byDate = useMemo(() => {
    const map = new Map(
      monthArrivals.map((a) => [String(a.date).slice(0, 10), a] as const),
    );
    if (todayArrival && todayArrival.date.startsWith(`${viewYear}-${String(viewMonth).padStart(2, '0')}`)) {
      const key = String(todayArrival.date).slice(0, 10);
      const existing = map.get(key);
      map.set(key, {
        ...todayArrival,
        departureTime: todayArrival.departureTime ?? existing?.departureTime,
        departureType: todayArrival.departureType ?? existing?.departureType,
      });
    }
    return map;
